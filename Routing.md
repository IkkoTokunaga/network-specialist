...
```
## 攻撃者目線
ルーティングの改ざんは既に侵入が完了し、管理者権限が奪取できている状態で選択肢として挙がる
MITM（中間者攻撃）に使われ、攻撃者によって乗っ取られているデバイスへルーティングさせることで通信を傍受できる
また、デフォルトゲートウェイを乗っ取ると全ての通信を傍受できるがその分発覚のリスクは高くなる
## 動的ルーティングとルーティングプロトコル
ルータ同士が定期的に広告（advertise）
受け取った経路を評価
+ ホップ数
+ 大域
+ 遅延
